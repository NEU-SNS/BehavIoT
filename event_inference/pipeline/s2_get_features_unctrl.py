...
def main():
    ...
    for device in dict_dec:
        ...
        if len(results) > 0: # list of dataframe
...
def run(paras_list, results):
    for paras in paras_list:
        ...
        tmp_data = load_features_per_exp(full_dec_file, device, state, event, mac_dic)
        if tmp_data is None or len(tmp_data) == 0:
            continue
        results.append(tmp_data)


def load_features_per_exp(dec_file, device_name, state, event, mac_dic):
    
    
    #Attempt to extract data from input files if not in previously-generated cache files
    feature_data = extract_features(dec_file, device_name, state, event, mac_dic)
    if feature_data is None or len(feature_data) == 0: #Can't extract from input files
        print('No data or features from %s' % dec_file)
        return

    return feature_data # dataframe


#Create CSV cache file
def extract_features(flow_burst, device_name, state, event, mac_dic):
    col_feat = cols_feat
    # original:
    ...
    pd_obj_all = pd.DataFrame(flow_burst, columns=header)
    ...
    if state == "power":
        return
    # print(pd_obj)
    pd_obj.frame_len = pd_obj.frame_len.astype(int)
    pd_obj.ts = pd_obj.ts.astype(float)
    pd_obj.ts_delta = pd_obj.ts_delta.astype(float)
    # print(pd_obj.frame_len)
    # print(skew(pd_obj.frame_len))
    # exit(1)
    ...
    d = compute_tbp_features(pd_obj, device_name, state, event, mac_dic)
    feature_data = feature_data.append(pd.DataFrame(data=[d], columns=col_feat))
    return feature_data


#Use Pandas to perform stat analysis on raw data
def compute_tbp_features(pd_obj, device_name, state, event, mac_dic):
    """
    Inputs:
        pd_obj : pandas object of the flow
        device name
        state (useless in unctrl setting: "unctrl")
        event (useless in unctrl setting: "unctrl")
    Return:
    ...
    for i, j, m in zip(pd_obj.ip_src, pd_obj.ip_dst, pd_obj.mac_addr):
        if m == my_device_mac:
            my_device_addr = j
            ...
            if m in mac_dic.values(): # local 
                ...
                external_destination_addr = ''
            elif ipaddress.ip_address(j).is_private==True or j=="129.10.227.248" or j=="129.10.227.207":    # router IPs
    ...
    for i, j, f_len in zip(pd_obj.ip_src, pd_obj.ip_dst, pd_obj.frame_len):
        ...
        # elif (regex.match(i) or (i == "129.10.227.248") or (i == "129.10.227.207")) and (regex.match(j) or (j == "129.10.227.248") or (j== "129.10.227.207")):
        elif pd_obj.host == 'local':
            network_local += 1
        else:
            # print(i,j,my_device_addr)
            # exit(1)
            pass
    ...
    d = [ meanBytes, minBytes, maxBytes, medAbsDev, skewL, kurtL, meanTBP, varTBP, medTBP,
         ...
         network_in_local, network_out_local, meanBytes_out_external,
         meanBytes_in_external, meanBytes_out_local, meanBytes_in_local, device_name, state, event, start_time, ";".join([x for x in protocol if x!= ""]), host_output ]
